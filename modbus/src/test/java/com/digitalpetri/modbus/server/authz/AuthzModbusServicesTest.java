...
class AuthzModbusServicesTest {
  ...
  private final AuthzHandler readAuthzHandler = new ReadWriteAuthzHandler() {
    @Override
    protected AuthzResult authorizeRead(int unitId, AuthzContext authzContext) {
      ...
      return clientRole.equals("ReadOnly") || clientRole.equals("ReadWrite")
          ? AuthzResult.AUTHORIZED : AuthzResult.NOT_AUTHORIZED;
    }
  ...
  private final AuthzModbusServices services = new AuthzModbusServices(
      readAuthzHandler,
  ...
  void testWriteSingleRegister() {
    ...
    assertDoesNotThrow(() -> {
      services.writeSingleRegister(writeOnlyContext, 1, new WriteSingleRegisterRequest(0, 1234));
    });
    assertDoesNotThrow(() -> {
      services.writeSingleRegister(readWriteContext, 1, new WriteSingleRegisterRequest(0, 1234));
    });
  }
  ...
  void testWriteMultipleCoils() {
    assertThrows(ModbusException.class, () -> {
      services.writeMultipleCoils(readOnlyContext, 1,
          new WriteMultipleCoilsRequest(0, 1, new byte[]{0}));
    });
    assertDoesNotThrow(() -> {
      services.writeMultipleCoils(writeOnlyContext, 1,
          new WriteMultipleCoilsRequest(0, 1, new byte[]{0}));
    });
    assertDoesNotThrow(() -> {
      services.writeMultipleCoils(readWriteContext, 1,
          new WriteMultipleCoilsRequest(0, 1, new byte[]{0}));
    });
  }
  ...
  void testWriteMultipleRegisters() {
    assertThrows(ModbusException.class, () -> {
      services.writeMultipleRegisters(readOnlyContext, 1,
          new WriteMultipleRegistersRequest(0, 1, new byte[]{0, 0}));
    });
    assertDoesNotThrow(() -> {
      services.writeMultipleRegisters(writeOnlyContext, 1,
          new WriteMultipleRegistersRequest(0, 1, new byte[]{0, 0}));
    });
    assertDoesNotThrow(() -> {
      services.writeMultipleRegisters(readWriteContext, 1,
          new WriteMultipleRegistersRequest(0, 1, new byte[]{0, 0}));
    });
  }
  ...
  void testMaskWriteRegister() {
    assertThrows(ModbusException.class, () -> {
      services.maskWriteRegister(readOnlyContext, 1,
          new MaskWriteRegisterRequest(0, 0xFFFF, 0x0000));
    });
    assertDoesNotThrow(() -> {
      services.maskWriteRegister(writeOnlyContext, 1,
          new MaskWriteRegisterRequest(0, 0xFFFF, 0x0000));
    });
    assertDoesNotThrow(() -> {
      services.maskWriteRegister(readWriteContext, 1,
          new MaskWriteRegisterRequest(0, 0xFFFF, 0x0000));
    });
  }
  ...
  void testReadWriteMultipleRegisters() {
    assertThrows(ModbusException.class, () -> {
      services.readWriteMultipleRegisters(readOnlyContext, 1,
          new ReadWriteMultipleRegistersRequest(0, 1, 0, 1, new byte[]{0, 0}));
    });
    assertThrows(ModbusException.class, () -> {
      services.readWriteMultipleRegisters(writeOnlyContext, 1,
          new ReadWriteMultipleRegistersRequest(0, 1, 0, 1, new byte[]{0, 0}));
    });
    assertDoesNotThrow(() -> {
      services.readWriteMultipleRegisters(readWriteContext, 1,
          new ReadWriteMultipleRegistersRequest(0, 1, 0, 1, new byte[]{0, 0}));
    });
  }
